...
2. Создайте сервисный аккаунт и ключ. [service_account_key_file](https://terraform-provider.yandexcloud.net).
   **+**
3. Сгенерируйте новый или используйте свой текущий ssh-ключ. Запишите его открытую(public) часть в переменную **vms_ssh_root_key**. 
   *(В проекте задана vms_ssh_root_key, она выполняет ту же роль.)*
   **+**
4. Инициализируйте проект, выполните код. Исправьте намеренно допущенные синтаксические ошибки. Ищите внимательно, посимвольно. Ответьте, в чём заключается их суть.
   Платформы *standar**t**-v**4*** не существует, она заменена на *standar**d**-v**1***.
   Только **standard-v1** поддерживается `core_fraction = 5`, поэтому выбрана она.
   Все платформы поддерживают минимум 2 ядра, поэтому `core_fraction = 5`.
5. Подключитесь к консоли ВМ через ssh и выполните команду ``` curl ifconfig.me```.
Примечание: К OS ubuntu "out of a box, те из коробки" необходимо подключаться под пользователем ubuntu: ```"ssh ubuntu@vm_ip_address"```. Предварительно убедитесь, что ваш ключ добавлен в ssh-агент: ```eval $(ssh-agent) && ssh-add``` Вы познакомитесь с тем как при создании ВМ создать своего пользователя в блоке metadata в следующей лекции.;
   - Доделать.
6. Ответьте, как в процессе обучения могут пригодиться параметры ```preemptible = true``` и ```core_fraction=5``` в параметрах ВМ.
* `preemptible = true` - включает прерывания ВМ. Это в несколько раз сокращает стимость ВМ.
* `core_fraction=5` - процент использования процессорного времени 5%. Это наименьшее значение, поэтому цена цена ВМ тоже снижается.
