...
module "analytics_vm" {
  ...
  network_id     = data.terraform_remote_state.module.vpc_dev.network.id
  subnet_zones   = [data.terraform_remote_state.var.default_zone]
  subnet_ids     = [data.terraform_remote_state.module.vpc_dev.subnets[0].id]
...
module "db_cluster" {
  ...
  network_id   = local.db_cluster_ha ? data.terraform_remote_state.module.vpc_prod.network.id : data.terraform_remote_state.module.vpc_dev.network.id
  hosts        = local.db_cluster_ha ? [{ 
    subnet_id = data.terraform_remote_state.module.vpc_prod.subnets[0].id, 
    zone = data.terraform_remote_state.module.vpc_prod.subnets[0].zone
  }, { 
    subnet_id = data.terraform_remote_state.module.vpc_prod.subnets[1].id, 
    zone = data.terraform_remote_state.module.vpc_prod.subnets[1].zone
  }] : [{ 
    subnet_id = data.terraform_remote_state.module.vpc_prod.subnets[0].id, 
    zone = data.terraform_remote_state.module.vpc_prod.subnets[0].zone
  }]
